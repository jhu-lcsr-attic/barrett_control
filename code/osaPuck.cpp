...
osaCANBusFrame::ID osaPuck::CANID( osaPuck::ID id )
{ return (osaCANBusFrame::ID)(0x1F & id); }

// STATIC return true if the CAN frame is a "set" command (a command has a 
// message of the form
// [1*** ****][**** ****]...[**** ****]
// thus we test if the MSB of the first byte is set
bool osaPuck::IsSetFrame( const osaCANBusFrame& canframe ){
...
osaPuck::ID osaPuck::DestinationID( osaCANBusFrame::ID cid )
{ return (osaPuck::ID)( cid & 0x1F); }

// STATIC returns the destination of a CAN frame
osaPuck::ID osaPuck::DestinationID( const osaCANBusFrame& canframe )
...
osaPuck::ID osaPuck::OriginID( osaCANBusFrame::ID cid ) 
{ return (osaPuck::ID)((cid>>5) & 0x1F); }

// STATIC returns the origin of a CAN frame.
osaPuck::ID osaPuck::OriginID( const osaCANBusFrame& canframe ) 
...
osaPuck::Errno osaPuck::PackProperty( osaCANBusFrame& canframe, 
				      Barrett::Command cmd,
				      Barrett::ID propid,
				      Barrett::Value propval ){
